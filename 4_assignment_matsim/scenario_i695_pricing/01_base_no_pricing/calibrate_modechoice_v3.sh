...
for PASS in $(seq 1 $MAXPASS); do
    ...
    "$JAVA" -Xmx8g -Dmodechoice=true \
        ...
        > "output_calib_v3/pass${PASS}.log" 2>&1
    RC=$?
    ...
    cp "$OUT/modestats.csv" "output_calib_v3/pass${PASS}_modestats.csv"
    rm -rf "$OUT/ITERS"
    [ $PASS -ge 2 ] && rm -rf "output_calib_v3/pass$((PASS-1))/output_plans.xml.gz"
    ...
    if awk "BEGIN{exit !(${CARG} <= ${CONV_CAR} && ${RIDEG} <= ${CONV_RIDE})}"; then
        ...
        echo "$CAR $PT $RIDE $WALK $BIKE" > output_calib_v3/FROZEN_ASCS.txt
        exit 0
    fi
